...
export function AdminAddClientModal({ onClienteAdicionado }: AdminAddClientModalProps) {
  const { user } = useAuth()
...
Seja bem-vindo! 💪`
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log("🔵 [AdminAddClientModal] === INICIANDO PROCESSO ===")
      ...
      const clienteData = {
        ...
        vendedor: 'Admin' // Diferença principal: sempre "Admin"
      }

      console.log("🔵 [AdminAddClientModal] Dados para adicionar:", clienteData)

      const result = await onClienteAdicionado(clienteData)
      
      if (result && result.success) {
        console.log("🟢 [AdminAddClientModal] === CLIENTE CRIADO COM SUCESSO ===")
        console.log("🟢 [AdminAddClientModal] Resultado:", result)
        ...
        console.log("🟢 [AdminAddClientModal] Preparando modal de instruções...")
        ...
        setTimeout(() => {
          console.log("🟢 [AdminAddClientModal] === ABRINDO MODAL DE INSTRUÇÕES ===")
          setShowInstructions(true)
        }, 300)
        
      } else {
        console.error("❌ [AdminAddClientModal] Resultado indica falha:", result)
        toast({
          title: "Erro",
      ...
      console.error('💥 [AdminAddClientModal] Erro ao adicionar cliente:', error)
      toast({
        title: "Erro",
  ...
  return (
    <>
      ...
      <ClientInstructionsModal
        ...
        onClose={() => {
          console.log("🔵 [AdminAddClientModal] === FECHANDO MODAL DE INSTRUÇÕES ===")
