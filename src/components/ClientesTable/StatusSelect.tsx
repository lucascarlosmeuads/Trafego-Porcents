...
export function StatusSelect({ 
  ...
  const selectContent = (
    <Select value={value} onValueChange={onValueChange} disabled={disabled || isUpdating}>
      <SelectTrigger className={`${compact ? 'h-6 text-xs w-fit max-w-[100px]' : 'h-8 w-fit max-w-[140px]'} bg-background/50 backdrop-blur-sm text-white border-border/50 hover:border-border transition-all duration-200`}>
        <SelectValue>
          <div className="flex items-center gap-1">
            {isUpdating && <Loader2 className="h-3 w-3 animate-spin" />}
            <div 
              className={`${getModernStatusStyle(value)} ${compact ? 'text-xs px-2 py-0.5 rounded-lg' : 'text-xs px-3 py-1 rounded-lg'} font-medium transition-all duration-200 hover:scale-105`}
            >
              {displayValue}
            </div>
          </div>
