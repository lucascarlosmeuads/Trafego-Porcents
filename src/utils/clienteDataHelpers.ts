...
export const ensureClienteExists = async (emailCliente: string, nomeCliente?: string) => {
  ...
  try {
    ...
    // Cliente não existe - NÃO criar automaticamente
    console.log('⚠️ [ensureClienteExists] Cliente não encontrado na tabela todos_clientes')
    console.log('❌ [ensureClienteExists] NÃO criando registro automático para evitar clientes fantasmas')
    return false
