...
-- 1) Garantir o trigger de processamento automático (antes do INSERT em kiwify_webhook_logs)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
...
-- 2) Reprocessar webhooks pagos/aprovados de 10 e 11/08
...
-- 4) Diagnóstico: webhooks pagos de 10/08 sem lead correspondente
WITH paid AS (
  SELECT 
    id AS log_id,
...
-- 5) Diagnóstico: webhooks pagos de 11/08 sem lead correspondente
WITH paid AS (
  SELECT 
    id AS log_id,
    lower(email_comprador) AS email_norm,
